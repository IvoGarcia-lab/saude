...
export default function DashboardPage() {
  ...
  // ─── Compute today's stats reactively from shared events context ───
  const { caloriesConsumed, proteinConsumed, carbsConsumed, fatConsumed } = useMemo(() => {
    ...
    // Sum from today's completed meal events
    const todayCompletedMeals = events.filter(
      e => e.dateStr === todayStr && e.type === 'meal' && e.completed
    );

    todayCompletedMeals.forEach(m => {
    ...
    return { caloriesConsumed: kcal, proteinConsumed: prot, carbsConsumed: carbs, fatConsumed: fat };
  }, [events]);
